...
function SelectDate(obj,strFormat,x,y,oT)   
{   
...
Calendar.prototype.draw = function() {   
  ...
  if(cal.onlyTime === 'time') {
	mvAry[mvAry.length]  = 'display:none;">';
  }else {
	mvAry[mvAry.length]  = '">';
  }    
  mvAry[mvAry.length]  = '      <tr>';   
  mvAry[mvAry.length]  = '        <th align="left" width="16px"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;';   
  if(cal.onlyTime === 'time') {
	mvAry[mvAry.length]  = 'display:none;';
  }else if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年时隐藏“月”   
  mvAry[mvAry.length]  ='" name="prevMonth" type="button" id="prevMonth" value="&lt;" /></th>';   
  mvAry[mvAry.length]  = '        <th align="center" nowrap="nowrap"><select name="calendarYear" id="calendarYear" style="font-size:12px;"></select><select name="calendarMonth" id="calendarMonth" style="font-size:12px;';   
  if(cal.onlyTime === 'time') {
	mvAry[mvAry.length]  = 'display:none;';
  }else if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年时隐藏“月”   
  mvAry[mvAry.length]  = '"></select></th>';   
  mvAry[mvAry.length]  = '        <th align="right" width="16px"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;';   
  if(cal.onlyTime === 'time') {
	mvAry[mvAry.length]  = 'display:none;';
  }else if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年时隐藏“月”   
  ...
  if(cal.onlyTime === 'time') {
	mvAry[mvAry.length]  = 'display:none;';
  }else if(calendar.DateMode >= pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年、月时隐藏“天”   
...
//2009-03-03 添加的代码，放置时间的行   
  mvAry[mvAry.length]  = '<table><tr style="';   
